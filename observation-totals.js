...
function grabCenterObservationTotal( centers, baseDate, endDate ) {
    return new Promise( ( resolve, reject ) => {
        ...
        centers.forEach( ( center ) => {
            const url = `http://dev.api.growthchart.me/observations/center/${center.id}/date/${baseDate}/${endDate}`;
            // Push on the promise
        ...
        Promise.all( observationPromises ).then( ( results ) => {
            ...
            for ( let i = 0; i < centers.length; i++ ) {
                ...
                csvRecords.push( {
                    ...
                    observationTotal: results[ i ].observationsCount === undefined ? 0 : results[ i ].observationsCount
                } );
            }
            resolve( csvRecords );
        } ).catch( ( err ) => {
...
// Date Range, Center Name, Observation Count
